...
LunarVape:Clean(gameCamera:GetPropertyChangedSignal('ViewportSize'):Connect(function()
  if LunarVape.Modules.Themes and LunarVape.Modules.Themes.Enabled then
    task.wait(0.1)
    LunarVape.Modules.Themes:Toggle()
    LunarVape.Modules.Themes:Toggle()
  end
end))
...
run(function()
  ...
  bd = setmetatable({
    AttackArgs = { 'target_entity_id', 'is_crit', 'weapon_name', 'extra' },
...
run(function()
  LunarVape.Categories.Blatant:CreateModule({
    Name = 'AntiHit',
    Function = function(callback)
      if callback and LunarVape.Modules.Killaura and not LunarVape.Modules.Killaura.Enabled then
        LunarVape.Modules.Killaura:Toggle()
      end
    end,
    Tooltip = 'Teleport up and back down to dodge attacks.'
  })
end)
...
run(function()
  ...
  local ParticleSize
  local LegitAura
  local Particles, Boxes, AutoHitReset, AttackDelay, SwingDelay, ClickDelay, DodgeDelay = {}, {}, {}, tick(), tick(),
      tick(), tick()
  ...
  local block_sword = function(toggle, tool_name)
    bd.ToolService:ToggleBlockSword(toggle, tool_name)
    bd.ViewmodelController:ToggleLoopedAnimation(tool_name, toggle)
  end
  ...
  Killaura = LunarVape.Categories.Blatant:CreateModule({
    ...
    Function = function(callback)
      if callback then
        ...
        repeat
          ...
          if tool and tool:HasTag('Sword') then
            local plrs = entitylib.AllPosition({
              Range = SwingRange.Value or math.huge,
              Wallcheck = nil,
              Part = 'RootPart',
              Players = Targets.Players.Enabled,
              NPCs = Targets.NPCs.Enabled,
              Limit = Max.Value,
              IgnoreY = LunarVape.Modules.AntiHit.Enabled
            })

            if #plrs > 0 then
              local selfpos = entitylib.character.RootPart.Position
              selfpos = Vector3.new(selfpos.X, 0, selfpos.Z)
              local localfacing = entitylib.character.RootPart.CFrame.LookVector * Vector3.new(1, 0, 1)

              if AutoB.Enabled and not bd.Entity.LocalEntity.IsBlocking then
                block_sword(true, tool.Name)
              end
              for _, v in next, plrs do
                if bd.Entity.LocalEntity.State.TeamColour == Color3.new(1, 1, 1) and v.RootPart.Position.Y > 0 then continue end
                local delta = Vector3.new(v.RootPart.Position.X, 0, v.RootPart.Position.Z) - selfpos
                ...
                if Block.Enabled and LegitAura.Enabled then
                  if bd.Entity.LocalEntity.IsBlocking then continue end
                end

                if delta.Magnitude > AttackRange.Value then continue end
                if AttackDelay < tick() then
                  if LunarVape.Modules.AntiHit.Enabled and not LegitAura.Enabled and DodgeDelay < tick() then
                    local ping = 0.1
                    if LunarVape.ThreadFix then
                      setthreadidentity(8)
                      ping = math.floor(tonumber(game:GetService('Stats'):FindFirstChild('PerformanceStats').Ping
                      :GetValue())) / 1000
                    end
                    local hrp = entitylib.character and entitylib.character.RootPart
                    local vhrp = v.Character and v.Character.HumanoidRootPart
                    hrp.CFrame = CFrame.lookAt(
                      hrp.Position - Vector3.new(0, (hrp.Position.Y - vhrp.Position.Y) or 0, 0),
                      hrp.Position + hrp.CFrame.LookVector,
                      hrp.Position + hrp.CFrame.UpVector
                    )
                    hrp.AssemblyLinearVelocity = Vector3.new(hrp.Velocity, 0, hrp.Velocity)

                    task.spawn(function()
                      task.wait(0.07 + ping)
                      if not hrp then return end
                      hrp.CFrame = CFrame.lookAt(
                        hrp.Position + Vector3.new(0, math.random(50, 100), 0),
                        hrp.Position + hrp.CFrame.LookVector,
                        hrp.Position + hrp.CFrame.UpVector
                      )
                      hrp.AssemblyLinearVelocity = Vector3.new(hrp.Velocity, 0, hrp.Velocity)
                    end)
                    if not AutoHitReset[v.Character.Name] then
                      AutoHitReset[v.Character.Name] = v.Humanoid.Died:Once(function()
                        if not vhrp or not hrp then return end
                        hrp.CFrame = CFrame.lookAt(
                          hrp.Position - Vector3.new(0, (hrp.Position.Y - vhrp.Position.Y) or 0, 0),
                          hrp.Position + hrp.CFrame.LookVector,
                          hrp.Position + hrp.CFrame.UpVector
                        )
                        if AutoHitReset[v.Character.Name] then
                          AutoHitReset[v.Character.Name] = nil
                        end
                      end)
                    end
                    DodgeDelay = tick() + 0.17 + ping
                  end
                  ...
                  if bdent then
                    bd.Call(bd.Blink.item_action.attack_entity.fire, {
                      ...
                      is_crit = LunarVape.Modules.Criticals and LunarVape.Modules.Criticals.Enabled and true or
                          entitylib.character.RootPart.AssemblyLinearVelocity.Y < 0,
              ...
              if AutoB.Enabled and bd.Entity.LocalEntity.IsBlocking and tick() < AttackDelay + 0.05 then
                block_sword(false, tool.Name)
              end
            end
          end
        ...
        if LunarVape.Modules.AntiHit and LunarVape.Modules.AntiHit.Enabled then LunarVape.Modules.AntiHit:Toggle() end
        for _, v in next, AutoHitReset do if v then v:Disconnect() end end
      end
    end,
  ...
  AutoB = Killaura:CreateToggle({ Name = 'Auto Block' })
...
run(function()
  ...
  local function nearCorner(poscheck, pos)
    ...
    return Vector3.new(math.clamp(check.X, startpos.X, endpos.X), math.clamp(check.Y, startpos.Y, endpos.Y),
          ...
          if entitylib.isAlive then
            ...
            if tool then
              ...
              for i = Expand.Value, 1, -1 do
                local currentpos = roundPos(root.Position -
                  Vector3.new(0,
                    entitylib.character.HipHeight +
                    (Downwards.Enabled and inputService:IsKeyDown(Enum.KeyCode.LeftShift) and 4.5 or 1.5), 0) +
